...
/**
 * Get hex color value from audience slug
 * @param {string} audienceSlug - The audience identifier
 * @returns {string} The hex color value
 */
export const getHexByAudience = (audienceSlug) => {
  const theme = getThemeByAudience(audienceSlug);
  return getHexByTheme(theme);
};
 ...
 * @returns {string} A random theme name (red, blue, yellow)
 */
...
/**
 * List all available brand colors and their properties
 * Useful for rendering color swatches or documentation
 * @returns {Array} Array of color objects with theme, hex, and tailwind properties
 */
export const getAllBrandColors = () => {
  return Object.entries(THEME_NAMES).map(([key, theme]) => ({
    name: key.toLowerCase(),
    theme,
    hex: COLOR_HEX[theme],
    tailwind: COLOR_TAILWIND[theme],
  }));
};
...
export const isValidTheme = (theme) => {
  ...
  return THEME_NAMES[theme.toUpperCase()] !== undefined;
};
