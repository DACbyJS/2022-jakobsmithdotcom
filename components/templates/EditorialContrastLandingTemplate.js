    ...
    <>
      ...
      <section className="bg-js-black text-js-white px-4 sm:px-6 lg:px-8 pt-16 sm:pt-20 pb-14 sm:pb-18">
        <div className="mx-auto max-w-[1100px] flex flex-col gap-y-4">
          <p className={`font-overpass-mono uppercase tracking-[0.22em] text-xs sm:text-sm mb-6 pl-2 ${accentColorClass}`}>
            {hero.eyebrow}
          </p>
          <h1 className="leading-[1.02] flex flex-col gap-y-4">
            ...
            <span className={`block font-overpass-mono uppercase tracking-[0.08em] text-[1.35rem] sm:text-[1.7rem] lg:text-[2.1rem] mt-2 pl-2 ${accentColorClass}`}>
              {hero.lines[1]}
            </span>
      ...
      <section className="bg-js-white text-js-black px-4 sm:px-6 lg:px-8 py-12 sm:py-20">
        <div className="mx-auto max-w-[1100px] space-y-16">
          <NextIntersectionObserver thresholdValue={0.2} classes="animate-init fade-up-init" topIn="fade-up-animate">
            <div>
              <h2 className="font-js-math text-[2.1rem] sm:text-[2.7rem]">{sections.processTitle}</h2>
              <ol className="mt-6 grid gap-4 sm:grid-cols-3">
                {sections.processSteps.map((step, index) => (
                  <li key={step.id} className="border-2 border-js-black p-5 bg-js-white">
                    <p className="font-overpass-mono text-xs tracking-[0.16em] uppercase">Step {index + 1}</p>
          ...
          <NextIntersectionObserver thresholdValue={0.2} classes="animate-init fade-up-init" topIn="fade-up-animate">
            <div>
              <h2 className="font-js-math text-[2.1rem] sm:text-[2.7rem]">{sections.buildTitle}</h2>
          ...
          <NextIntersectionObserver thresholdValue={0.2} classes="animate-init fade-up-init" topIn="fade-up-animate">
            <div>
              <h2 className="font-js-math text-[2.1rem] sm:text-[2.7rem]">{sections.includedTitle}</h2>
              <ul className="mt-6 grid gap-3 sm:grid-cols-2">
                {sections.includedItems.map((item) => (
                  <li key={item.id} className="border-2 border-js-black px-4 py-3 font-overpass text-lg bg-white">
                    {item.text}
                  </li>
                ))}
              </ul>
          ...
          <NextIntersectionObserver thresholdValue={0.2} classes="animate-init fade-up-init" topIn="fade-up-animate">
            <div>
              <h2 className="font-js-math text-[2.1rem] sm:text-[2.7rem]">{sections.pricingTitle}</h2>
          ...
          <NextIntersectionObserver thresholdValue={0.2} classes="animate-init fade-up-init" topIn="fade-up-animate">
            <div className="border-2 border-js-black p-6 sm:p-8 bg-js-black text-js-white">
              <h2 className="font-js-math text-[2rem] sm:text-[2.5rem]">{cta.title}</h2>
              <p className="font-overpass text-lg mt-3 text-js-white">{cta.body}</p>
